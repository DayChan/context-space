...
export interface TraexProviderOptions {
  runner?: CodexExecRunner;
  executable?: string;
  environment?: NodeJS.ProcessEnv;
}

export function buildTraexArguments(
  schemaPath: string,
  resultPath: string,
  model: string | null
): string[] {
...
function parseJsonLines(stdout: string): {
  ...
  for (const line of stdout.split(/\r?\n/).filter(Boolean)) {
    ...
    try {
      ...
      throw new AnalysisProviderError("provider_failed", "traex 返回了无效 JSONL 事件");
    }
    const item =
...
export class TraexProvider implements AnalysisProvider {
  readonly id = "traex";
  private readonly runner: CodexExecRunner;
  private readonly executable: string;
  private readonly environment: NodeJS.ProcessEnv;

  constructor(options: TraexProviderOptions = {}) {
    this.environment = options.environment ?? process.env;
    this.executable = options.executable ?? "traex";
    this.runner =
      options.runner ??
      new NodeCodexExecRunner({
        executable: this.executable,
        displayName: "traex CLI",
  ...
  async analyze(
    ...
    const execution = await this.runner.run({
      executable: this.executable,
      args: buildTraexArguments(schemaPath, resultPath, request.model),
    ...
    try {
      ...
      throw new AnalysisProviderError(
        "invalid_output",
        `traex 没有生成最终响应：${sanitizedErrorMessage(error)}`
      );
    }
    if (resultSize > request.maxOutputBytes) {
      throw new AnalysisProviderError("output_too_large", "traex 最终响应超过大小限制");
    }
    const finalResponse = await readFile(resultPath, "utf8");
    if (!finalResponse.trim()) {
      throw new AnalysisProviderError("invalid_output", "traex 最终响应为空");
    }
    return {
      finalResponse,
      model: request.model,
