...
消息搜索的成功响应不代表搜索索引已经对该时间段收敛。每次增量同步 MUST 完整覆盖上次成功游标至本次同步时间；同时将查询起点向前扩展到近期滚动校准下界，重复读取可能延迟可见的记录。有效起点取“上次成功游标”和“本次时间减去校准时长”中更早者，因此校准不得截短停机或长间隔形成的增量区间。稳定来源 ID 负责来源去重，只有新增或分析相关内容发生变化的来源才进入幂等分析队列。
...
- [飞书搜索结果可能延迟可见] → 游标覆盖完整增量区间，并用可配置的近期滚动窗口重复校准；重复来源不重新分析。
