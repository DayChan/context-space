...
Todo Markdown 与 Meego 来源仍是事项真相，Agent 会话只是一次执行尝试。Agent 产生的代码、消息和完成判断不得反向覆盖 Todo 或 Meego。运行时支持 Codex、TraeX 和 Claude，但领域层不得绑定任一 SDK 或 CLI 的具体类型。
...
领域服务依赖 `AgentRuntime` 接口：根据 Session 固化的 Agent 类型创建或恢复线程、流式运行一轮和取消。`CodexAgentRuntime` 使用 `runStreamed()`；TraeX 与 Claude 使用各自的非交互 CLI 和持久会话参数。运行时事件统一标准化后写入仓库。测试注入 Fake Runtime，不调用真实 Agent 或真实用户仓库。

Agent 类型和可空模型覆盖在会话创建时固化。空模型不向 SDK 或 CLI 传模型参数，由对应 Agent 使用自身默认模型。同一会话不得切换 Agent，因为各运行时的 Thread/Session ID、上下文格式和权限语义不兼容；用户需要更换 Agent 时应创建新会话。

每轮最终输出使用由 Zod 传输模型生成的 JSON Schema。`message`、`outcome` 和 `confirmation` 都是必填字段；不需要确认时 `confirmation` 为 `null`。允许的 outcome 为 `completed`、`needs_confirmation`、`awaiting_reply` 和 `blocked`。Zod 二次校验 outcome 与 confirmation 的业务一致性，结构化结果决定注意力分类，不解析自然语言猜测。
