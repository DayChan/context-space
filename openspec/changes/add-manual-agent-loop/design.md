...
每轮最终输出使用由 Zod 传输模型生成的 OpenAI-compatible JSON Schema。`message`、`outcome` 和 `confirmation` 都是必填字段；不需要确认时 `confirmation` 为 `null`，避免 Structured Outputs 因可选 properties 被服务端拒绝。允许的 outcome 为 `completed`、`needs_confirmation`、`awaiting_reply` 和 `blocked`。Zod 二次校验 outcome 与 confirmation 的业务一致性，结构化结果决定注意力分类，不解析自然语言猜测。
