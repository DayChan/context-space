...
- `read_only`：工作目录为注册仓库真实路径，Codex 使用 `read-only` 沙箱，不创建分支或 worktree。
...
仓库作为 SQLite 实体管理。注册时服务端使用 `realpath`、`git rev-parse --show-toplevel` 和 `git rev-parse HEAD` 验证路径，拒绝非 Git 目录、重复真实路径和不存在路径。API 不把浏览器提交的路径直接作为命令片段，所有 Git 操作使用 `execFile` 参数数组。删除仓库注册不会删除磁盘仓库。
...
- `agent_repositories`：仓库身份、真实路径和 Git 元数据。
